...
public enum EnumMode {
    ...
    private EnumMode(int index, String name) {
        this(index, name, "pigeon.mode." + name, "pigeon.mode." + name + ".indicator", "pigeon.mode." + name + ".description");
    }
    ...
    public void onModeSet(AbstractPigeonEntity pigeon, EnumMode prev) {
        switch (prev) {
            default:
                pigeon.getNavigator().clearPath();
                pigeon.setAttackTarget(null);
                pigeon.setLastAttackedEntity(null);
                break;
        }
    }
