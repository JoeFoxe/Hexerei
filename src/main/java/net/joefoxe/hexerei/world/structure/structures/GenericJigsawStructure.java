...
public class GenericJigsawStructure extends AbstractBaseStructure<NoFeatureConfig> {
    ...
    public GenericJigsawStructure(ResourceLocation poolID, Lazy<Integer> structureSize, int centerOffset, int biomeRange,
                                  int structureBlacklistRange, int allowTerrainHeightRange, int terrainHeightRadius, int minHeightLimit,
                                  int fixedYSpawn, boolean useHeightmap, boolean cannotSpawnInWater)
    {
        ...
        ModStructures.HEXEREI_STRUCTURE_START_PIECES.add(startPool);
    }
        ...
        if(allowTerrainHeightRange != -1){
            ...
            for (int curChunkX = chunkX - terrainHeightRadius; curChunkX <= chunkX + terrainHeightRadius; curChunkX++) {
                for (int curChunkZ = chunkZ - terrainHeightRadius; curChunkZ <= chunkZ + terrainHeightRadius; curChunkZ++) {
                    int height = chunkGenerator.getHeight(curChunkX * 16, curChunkZ * 16, Heightmap.Type.WORLD_SURFACE_WG);
...
//                    new VillageConfig(() -> dynamicRegistryManager.getRegistry(Registry.JIGSAW_POOL_KEY).getOrDefault(startPool), structureSize.get()),
//                    chunkGenerator,
//                    structureManager,
//                    blockpos,
//                    this.components,
...
//                    Integer.MIN_VALUE);
            ...
            StructureUtils.centerAllPieces(blockpos, this.components);
