...
//        MutableRegistry<JigsawPattern> jigsawPoolRegistry = dynamicRegistries.getRegistry(Registry.JIGSAW_POOL_KEY);
//
...
//        if(startPool == null || startPool.getNumberOfPieces() == 0){
//            Hexerei.LOGGER.warn("Hexerei: Empty or nonexistent start pool: {}  Crash is imminent", startPool == null ? structureID : (startPool.getName() + " - " + structureID));
//        }
//
...
//        int pieceCenterX = (pieceBoundingBox.minX + pieceBoundingBox.minX) / 2;
//        int pieceCenterZ = (pieceBoundingBox.maxZ + pieceBoundingBox.minZ) / 2;
//        int pieceCenterY = useHeightmap
//                ? startPos.getY() + chunkGenerator.getNoiseHeightMinusOne(pieceCenterX, pieceCenterZ, Heightmap.Type.WORLD_SURFACE_WG)
//                : startPos.getY();
//
//        int yAdjustment = pieceBoundingBox.minY + startPiece.getGroundLevelDelta();
//        startPiece.offset(0, pieceCenterY - yAdjustment, 0);
//
...
//                boxOctree.addBox(AxisAlignedBB.toImmutable(pieceBoundingBox));
//                Entry startPieceEntry = new Entry(startPiece, new MutableObject<>(boxOctree), pieceCenterY + 80, 0);
//
//                Assembler assembler = new Assembler(jigsawPoolRegistry, jigsawConfig.func_236534_a_(), chunkGenerator, templateManager, components, random, requiredPieces, maxY, minY);
...
//                JigsawPiece poolElement = ((AbstractVillagePiece)piece).getJigsawPiece();
...
//                Direction direction = JigsawBlock.getJigsawAlignmentDirection(jigsawBlock.state);
...
//                    Hexerei.LOGGER.warn("Repurposed Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockPool, pieceBlueprint instanceof SingleJigsawPiece ? ((SingleJigsawPieceAccessor) pieceBlueprint).getTemplate().left().get() : "not a SingleJigsawPiece class");
//                    continue;
//                }
//
...
//                    Hexerei.LOGGER.warn("Repurposed Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockFallback, pieceBlueprint instanceof SingleJigsawPiece ? ((SingleJigsawPieceAccessor) pieceBlueprint).getTemplate().left().get() : "not a SingleJigsawPiece class");
//                    continue;
//                }
//
...
//                        parentOctree.setValue(new BoxOctree(AxisAlignedBB.toImmutable(pieceBoundingBox)));
//                    }
//                }
//                else {
...
//                    JigsawPiece generatedPiece = this.processList(new ArrayList<>(((JigsawPatternAccessor)poolOptional.get()).hexerei_getRawTemplates()), doBoundaryAdjustments, jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, octreeToUse, piece, depth, targetPieceBoundsTop);
...
//                this.processList(new ArrayList<>(((JigsawPatternAccessor)fallbackOptional.get()).hexerei_getRawTemplates()), doBoundaryAdjustments, jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, octreeToUse, piece, depth, targetPieceBoundsTop);
//            }
//        }
//
//        /**
...
//                            if (!tempCandidateBoundingBox.isVecInside(pieceCandidateJigsawBlock.pos.offset(JigsawBlock.getJigsawAlignmentDirection(pieceCandidateJigsawBlock.state)))) {
//                                return 0;
//                            } else {
...
//                                int heightAdjustement = Math.max(candidateHeightAdjustments + 1, adjustedCandidateBoundingBox.maxY - adjustedCandidateBoundingBox.minY);
//                                adjustedCandidateBoundingBox.maxY = adjustedCandidateBoundingBox.minY + heightAdjustement;
//                            }
//
...
//                            }
//
//                            AxisAlignedBB axisAlignedBB = AxisAlignedBB.toImmutable(adjustedCandidateBoundingBox);
//                            AxisAlignedBB axisAlignedBBDeflated = axisAlignedBB.shrink(0.25D); // Avoid any edge case weirdness if size is exact with bounding boxes.
//
//                            // debugging
...
////                                    Hexerei.LOGGER.warn(" Failed to spawn pieces. Parent: {} - General info: {} - Child: {} - General info: {} - Octree: {}", piece.getBoundingBox().toString(), piece.toString(), axisAlignedBBDeflated.toString(), candidatePiece.toString(), mutableObjectBoxOctree.toString());
////                                }
////                            }
//
