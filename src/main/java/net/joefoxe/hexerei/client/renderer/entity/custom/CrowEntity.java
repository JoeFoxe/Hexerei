...
import net.joefoxe.hexerei.message.MessageCrowDismount;
...
public class CrowEntity extends TameableEntity implements ITargetsDroppedItems {
    ...
    private static final DataParameter<Integer> COMMAND = EntityDataManager.createKey(CrowEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Optional<BlockPos>> PERCH_POS = EntityDataManager.createKey(CrowEntity.class, DataSerializers.OPTIONAL_BLOCK_POS);
    public static final ResourceLocation SCATTERS_CROWS = new ResourceLocation("hexerei", "scatters_crows");
    public static final ResourceLocation CROW_FEARS = new ResourceLocation("hexerei", "crow_fears");
    public static final ResourceLocation CROW_FOODBLOCKS = new ResourceLocation("hexerei", "crow_foodblocks");
    public static final ResourceLocation CROW_FOODSTUFFS = new ResourceLocation("hexerei", "crow_foodstuffs");
    ...
    public CrowEntity(EntityType type, World worldIn) {
        super(type, worldIn);
...
//    }
    ...
    public static <T extends MobEntity> boolean canCrowSpawn(EntityType<CrowEntity> crow, IWorld worldIn, SpawnReason reason, BlockPos p_223317_3_, Random random) {
        ...
        if (this.isPassenger() && !entity.isAlive()) {
            ...
            if (this.isPassenger()) {
                ...
                if (!riding.isAlive() || rideCooldown == 0 && riding.isSneaking() || ((PlayerEntity) riding).isElytraFlying() || this.getAttackTarget() != null && this.getAttackTarget().isAlive()) {
                    this.dismount();
                    if (!world.isRemote) {
//                        Hexerei.sendMSGToAll(new MessageCrowDismount(this.getEntityId(), riding.getEntityId()));
                    }
                }
            }
        }else{
    ...
    public AgeableEntity createChild(ServerWorld serverWorld, AgeableEntity ageableEntity) {
        return ModEntityTypes.CROW.get().create(serverWorld);
    }
    ...
    private boolean isCrowEdible(ItemStack stack) {
        return stack.getItem().isFood() || ItemTags.getCollection().get(CROW_FOODSTUFFS).contains(stack.getItem());
    }
    ...
    private class AIWalkIdle extends Goal {
        ...
        public boolean shouldExecute() {
            if (this.crow.isBeingRidden() || CrowEntity.this.getCommand() == 1 || CrowEntity.this.aiItemFlag || (crow.getAttackTarget() != null && crow.getAttackTarget().isAlive()) || this.crow.isPassenger() || this.crow.isSitting()) {
                return false;
            } else {
        ...
        public boolean shouldContinueExecuting() {
            if (crow.aiItemFlag || crow.isSitting() || CrowEntity.this.getCommand() == 1) {
                return false;
            }
            if (flightTarget) {
    ...
    private class AIScatter extends Goal {
        ...
        AIScatter() {
            ...
            tag = EntityTypeTags.getCollection().get(SCATTERS_CROWS);
            this.theNearestAttackableTargetSorter = new AIScatter.Sorter(CrowEntity.this);
        ...
        @Override
        public boolean shouldExecute() {
            if (CrowEntity.this.isPassenger() || CrowEntity.this.aiItemFlag || CrowEntity.this.isBeingRidden() || CrowEntity.this.isTamed()) {
                return false;
            }
            if (!this.mustUpdate) {
        ...
        @Override
        public boolean shouldContinueExecuting() {
            return targetEntity != null && !CrowEntity.this.isTamed();
        }
    ...
    private class AIAvoidPumpkins extends Goal {
        ...
        public boolean shouldContinueExecuting() {
            return destinationBlock != null && isPumpkin(CrowEntity.this.world, destinationBlock.toMutable()) && isCloseToPumpkin(16);
        }

        public boolean isCloseToPumpkin(double dist) {
            return destinationBlock == null || CrowEntity.this.getDistanceSq(Vector3d.copyCentered(destinationBlock)) < dist * dist;
        }

        @Override
        public boolean shouldExecute() {
            if (CrowEntity.this.isTamed()) {
                return false;
            }
            if (this.runDelay > 0) {
                --this.runDelay;
                ...
                this.runDelay = 70 + CrowEntity.this.rand.nextInt(150);
                return this.searchForDestination();
            }
        }

        public void startExecuting() {
            CrowEntity.this.fleePumpkinFlag = 200;
            Vector3d vec = CrowEntity.this.getBlockInViewAway(Vector3d.copyCentered(destinationBlock), 10);
            if (vec != null) {
                flightTarget = vec;
                CrowEntity.this.setFlying(true);
                CrowEntity.this.getMoveHelper().setMoveTo(vec.x, vec.y, vec.z, 1F);
            }
        }
        ...
        public void tick() {
            if (this.isCloseToPumpkin(16)) {
                CrowEntity.this.fleePumpkinFlag = 200;
                if (flightTarget == null || CrowEntity.this.getDistanceSq(flightTarget) < 2F) {
                    Vector3d vec = CrowEntity.this.getBlockInViewAway(Vector3d.copyCentered(destinationBlock), 10);
                    if (vec != null) {
                        flightTarget = vec;
                        CrowEntity.this.setFlying(true);
        ...
        protected boolean searchForDestination() {
            ...
            BlockPos lvt_3_1_ = CrowEntity.this.getPosition();
                ...
                for (int lvt_6_1_ = 0; lvt_6_1_ < lvt_1_1_; ++lvt_6_1_) {
                    for (int lvt_7_1_ = 0; lvt_7_1_ <= lvt_6_1_; lvt_7_1_ = lvt_7_1_ > 0 ? -lvt_7_1_ : 1 - lvt_7_1_) {
                        for (int lvt_8_1_ = lvt_7_1_ < lvt_6_1_ && lvt_7_1_ > -lvt_6_1_ ? lvt_6_1_ : 0; lvt_8_1_ <= lvt_6_1_; lvt_8_1_ = lvt_8_1_ > 0 ? -lvt_8_1_ : 1 - lvt_8_1_) {
                            ...
                            if (this.isPumpkin(CrowEntity.this.world, lvt_4_1_)) {
        ...
        private boolean isPumpkin(World world, BlockPos.Mutable lvt_4_1_) {
            return BlockTags.getCollection().get(CROW_FEARS).contains(world.getBlockState(lvt_4_1_).getBlock());
        }
    ...
    private class AIDepositChests extends Goal {
        ...
        AIDepositChests() {
            this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE));
            this.theNearestAttackableTargetSorter = new AIDepositChests.Sorter(CrowEntity.this);
                ...
                public boolean apply(@Nullable ItemFrameEntity e) {
                    ...
                    if(entity != null){
                        ...
                        if(handler != null && handler.isPresent()){
                            return e.getDisplayedItem().isItemEqual(CrowEntity.this.getHeldItemMainhand());
                        }
                    }
                    return false;
                }
        ...
        @Override
        public boolean shouldExecute() {
            if (CrowEntity.this.isPassenger() || CrowEntity.this.aiItemFlag || CrowEntity.this.isBeingRidden() || CrowEntity.this.isSitting() || CrowEntity.this.getCommand() != 3) {
                return false;
            }
            if(CrowEntity.this.getHeldItemMainhand().isEmpty()){
                return false;
            }
            if (!this.mustUpdate) {
                long worldTime = CrowEntity.this.world.getGameTime() % 10;
            ...
            List<ItemFrameEntity> list = CrowEntity.this.world.getEntitiesWithinAABB(ItemFrameEntity.class, this.getTargetableArea(this.getTargetDistance()), this.targetEntitySelector);
            if (list.isEmpty()) {
                return false;
            } else {
                Collections.sort(list, this.theNearestAttackableTargetSorter);
                this.targetEntity = list.get(0);
                this.mustUpdate = false;
                CrowEntity.this.aiItemFrameFlag = true;
                return true;
            }
        }
        ...
        @Override
        public boolean shouldContinueExecuting() {
            return targetEntity != null && CrowEntity.this.getCommand() == 3 && !CrowEntity.this.getHeldItemMainhand().isEmpty();
        }

        public void resetTask() {
            flightTarget = null;
            this.targetEntity = null;
            CrowEntity.this.aiItemFrameFlag = false;
        }
        ...
        public void tick() {
            ...
            if (flightTarget != null) {
                CrowEntity.this.setFlying(true);
                if(CrowEntity.this.collidedHorizontally){
                    CrowEntity.this.getMoveHelper().setMoveTo(flightTarget.x, CrowEntity.this.getPosY() + 1F, flightTarget.z, 1F);
            ...
            if (targetEntity != null) {
                ...
                if (CrowEntity.this.getDistance(targetEntity) < 2.0F) {
                    try{
                        ...
                        if(handler.orElse(null) != null && cooldown == 0) {
                            ItemStack duplicate = CrowEntity.this.getHeldItem(Hand.MAIN_HAND).copy();
                            ...
                            if (!insertSimulate.equals(duplicate)) {
                                ...
                                if(shrunkenStack.isEmpty()){
                                    CrowEntity.this.setHeldItem(Hand.MAIN_HAND, ItemStack.EMPTY);
                                }else{
                                    CrowEntity.this.setHeldItem(Hand.MAIN_HAND, shrunkenStack);
                                }
        ...
        protected AxisAlignedBB getTargetableArea(double targetDistance) {
            Vector3d renderCenter = new Vector3d(CrowEntity.this.getPosX(), CrowEntity.this.getPosY(), CrowEntity.this.getPosZ());
