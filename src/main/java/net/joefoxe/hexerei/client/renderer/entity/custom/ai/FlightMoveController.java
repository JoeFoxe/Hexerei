...
public class FlightMoveController extends MovementController {
    ...
    public void tick() {
        ...
        if(parentEntity instanceof PigeonEntity)
        {
            PigeonEntity pigeonEntity = (PigeonEntity) parentEntity;
            if(pigeonEntity.timeFlying > 50)
                pigeonEntity.setMotion(pigeonEntity.getMotion().getX(), pigeonEntity.getMotion().getY() - 0.02f, pigeonEntity.getMotion().getZ());
        }
    }
